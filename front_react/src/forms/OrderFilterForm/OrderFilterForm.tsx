...
const OrderFilterForm: FC = () => {
  ...
  const { register, watch, reset } = useForm<IOrderFilterQuery>({
    defaultValues,
  });
  ...
  const debouncedFormSubmitHandler = useCallback(
    debounce((formData: IOrderFilterQuery) => {
  ...
  return (
    <form className={styles.form}>
      ...
      <input
        className={styles.form_item}
        type="date"
        {...register("lower_date", { required: false })}
        title={"Search after this date..."}
      />

      <label className={`${styles.my_order}  ${styles.form_item}`}>
        <SvgCheck
          className={[
            styles.check,
            watch().my_orders ? styles.check_show : "",
          ].join(" ")}
        />
        <span>My orders</span>
        <input type="checkbox" {...register("my_orders")} />
      </label>
      <FilterFormSelect
        register={register}
        field_name={"course"}
      ...
      <input
        className={styles.form_item}
        type="date"
        {...register("upper_date", { required: false })}
        title={"Search before this date..."}
      />
      <div
        onClick={formReset}
        className={`${styles.reset} ${styles.form_item}`}
      >
        <p>Reset</p>
      </div>
    </form>
