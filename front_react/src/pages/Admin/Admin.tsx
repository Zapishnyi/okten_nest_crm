...
const Admin: FC = memo(() => {
  ...
  useEffect(() => {
    ...
    } else if (!location.search.match(/^(?=.*\bsort=\b)(?=.*\bsortBy=\b).*/)) {
      setQuery(queryToSearchParams(initialUsersQuery), { replace: true });
      dispatch(
        UsersActions.getAllUsers(queryToSearchParams(initialUsersQuery))
      );
    } else {
  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.table_container}>
        {!!usersNoStatistic.length && (
          <Table<UserNoStatisticType> items={usersNoStatistic} />
        )}
      </div>
    </div>
  );
